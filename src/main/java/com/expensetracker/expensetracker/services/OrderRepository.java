...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Order t " +
            "WHERE t.orderType = 'EXPENSE' " +
    ...
    @Query("SELECT COUNT(t) FROM Order t WHERE t.date BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT SUM(t.amount) FROM Order t WHERE t.orderType = 'INCOME' AND t.date >= :startDate AND t.date <= :endDate")
    ...
    @Query("SELECT SUM(t.amount) FROM Order t WHERE t.date >= :startDate AND t.date <= :endDate AND t.orderType = 'EXPENSE'")
    ...
    @Query("SELECT COUNT(t) FROM Order t WHERE t.date >= :startDate AND t.date <= :endDate")
    int countTransactionsMonth(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);

    @Query("SELECT t FROM Order t WHERE t.description = :description")
